...
class MeshType:
    ...
    def check_lat_lon_dims (self):
        ...
        if self.lat_dims not in valid_dims:
            print('Unrecognized grid! The dimension of latitude should be either 1 or 2 but it is {}.'.format(self.lat_dims))
            
        if self.lon_dims not in valid_dims:
            print('Unrecognized grid! The dimension of longitude should be either 1 or 2 but it is {}.'.format(self.lon_dims))
    ...
    def create_artificial_mask (self):
        ...
        if self.lat_dims == 1:
            ...
            #-- 2D mask
            mask = np.ones(tuple(self.center_lats.sizes.values()),dtype=np.int8) #np.ones(center_lats.shape),dtype=np.int8)
    ...
    def calculate_corners(self):
        ...
        if self.lat_dims == 1:
            ...
            self.center_lat2d = da.broadcast_to(self.center_lats.values[None,:], (lons_size, lats_size))
            self.center_lon2d = da.broadcast_to(self.center_lons.values[:,None], (lons_size, lats_size))
        ...
        center_lat2d_ext = da.from_array(np.pad(self.center_lat2d.compute(), (1,1),  mode='reflect', reflect_type='odd'))

        #-- calculate corner lats
        upper_right = (center_lat2d_ext[1:-1,1:-1]+
                      center_lat2d_ext[0:-2,1:-1]+
                      center_lat2d_ext[1:-1,2:]+
                      center_lat2d_ext[0:-2,2:])/4.0
        upper_left = (center_lat2d_ext[1:-1,1:-1]+
                      center_lat2d_ext[0:-2,1:-1]+
                      center_lat2d_ext[1:-1,0:-2]+
                      center_lat2d_ext[0:-2,0:-2])/4.0
        lower_left = (center_lat2d_ext[1:-1,1:-1]+
                      center_lat2d_ext[1:-1,0:-2]+
                      center_lat2d_ext[2:,1:-1]+
                      center_lat2d_ext[2:,0:-2])/4.0
        lower_right = (center_lat2d_ext[1:-1,1:-1]+
                      center_lat2d_ext[1:-1,2:]+
                      center_lat2d_ext[2:,1:-1]+
                      center_lat2d_ext[2:,2:])/4.0

        self.corner_lat = da.stack([upper_left.T.reshape((-1,)).T, lower_left.T.reshape((-1,)).T, lower_right.T.reshape((-1,)).T, upper_right.T.reshape((-1,)).T], axis=1)

        # ------------------------------------------
        # calculate corner coordinates for longitude, counterclockwise order, imposing Fortran ordering
        center_lon2d_ext = da.from_array(np.pad(self.center_lon2d.compute(), (1,1),  mode='reflect', reflect_type='odd'))

        #-- calculate corner lons
        upper_right= (center_lon2d_ext[1:-1,1:-1]+
                      center_lon2d_ext[0:-2,1:-1]+
                      center_lon2d_ext[1:-1,2:]+
                      center_lon2d_ext[0:-2,2:])/4.0
        upper_left = (center_lon2d_ext[1:-1,1:-1]+
                      center_lon2d_ext[0:-2,1:-1]+
                      center_lon2d_ext[1:-1,0:-2]+
                      center_lon2d_ext[0:-2,0:-2])/4.0
        lower_left = (center_lon2d_ext[1:-1,1:-1]+
                      center_lon2d_ext[1:-1,0:-2]+
                      center_lon2d_ext[2:,1:-1]+
                      center_lon2d_ext[2:,0:-2])/4.0
        lower_right = (center_lon2d_ext[1:-1,1:-1]+
                      center_lon2d_ext[1:-1,2:]+
                      center_lon2d_ext[2:,1:-1]+
                      center_lon2d_ext[2:,2:])/4.0

        self.corner_lon = da.stack([upper_left.T.reshape((-1,)).T, lower_left.T.reshape((-1,)).T, lower_right.T.reshape((-1,)).T, upper_right.T.reshape((-1,)).T], axis=1)
    ...
    def write_to_esmf_mesh(self, filename, area=None):
        ...
        print ('corner_lon:', self.corner_lon.shape)
        print ('corner_lat:', self.corner_lat.shape)
        corner_pair = da.stack([self.corner_lon.T.reshape((-1,)).T, self.corner_lat.T.reshape((-1,)).T], axis=1)
        print (corner_pair)
        ...
        if node_coords.shape[0] != elem_conn_size:
            print('The size of unique coordinate pairs is {} but expected size is {}!'.format(node_coords.shape[0], elem_conn_size))
            print('Please check the input file or try to force double precision with --double option. Exiting ...')
            sys.exit(2)

        # create element connections
        corners = dd.concat([dd.from_dask_array(c) for c in [self.corner_lon.T.reshape((-1,)).T, self.corner_lat.T.reshape((-1,)).T]], axis=1)   
        ...
        out['origGridDims'] = xr.DataArray(np.array(self.center_lon2d.shape, dtype=np.int32),
                                           dims=('origGridRank'))
        #print (out['origGridDims'])
        out['nodeCoords'] = xr.DataArray(node_coords,
        ...
        out['numElementConn'] = xr.DataArray(4*np.ones(self.center_lon2d.size, dtype=np.int32),
                                             dims=('elementCount'),
                                             attrs={'long_name': 'Number of nodes per element'})

        center_coords = da.stack([self.center_lon2d.T.reshape((-1,)).T, 
                                  self.center_lat2d.T.reshape((-1,)).T], axis=1)
        print (center_coords)
        out['centerCoords'] = xr.DataArray(center_coords,
        ...
        # add mask
        out['elementMask'] = xr.DataArray(self.mask.T.reshape((-1,)).T,
                                          dims=('elementCount'),
        ...
        out.attrs = {'title': 'ESMF unstructured grid file for rectangular grid with {} dimension'.format('x'.join(list(map(str,self.center_lats.shape)))),
